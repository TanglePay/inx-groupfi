...
func (im *Manager) CalculateReputationScore(groupId [GroupIdLen]byte, mutedAddrSha256Hash [Sha256HashLen]byte) (float32, error) {
	...
	addresses, err := im.GetGroupMemberAddressesFromGroupId(groupId)
	if err != nil {
		return 0, err
	}
...
func (im *Manager) HandleUserMuteGroupMemberBasicOutputCreated(output *iotago.BasicOutput) {
	...
	for _, userMuteGroupMember := range userMuteGroupMembers {
		err := im.StoreUserMuteGroupMember(userMuteGroupMember)
		if err != nil {
			return
		}
	}
