...
func StoreRecentConsumedOutputId(addressSha256Hash [Sha256HashLen]byte, outputID [OutputIdLen]byte, im *Manager, shouldLog bool) error {
	if shouldLog {
	...
	if shouldLog {
		Logger.Infof("StoreRecentConsumedOutputId ... address sha256 hash:%s, count:%d", iotago.EncodeHex(addressSha256Hash[:]), count)
	}
	if err != nil {
