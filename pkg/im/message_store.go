...
/*
// mark inited
func (s *StoreHealthTracker) MarkInited() error {

	if err := s.store.Set([]byte("dbInited"), []byte{}); err != nil {
...
// is inited
func (s *StoreHealthTracker) IsInited() (bool, error) {

	contains, err := s.store.Has([]byte("dbInited"))
	if err != nil {
...
*/
// is inited
func (im *Manager) IsInited() (bool, error) {

	contains, err := im.imStore.Has([]byte("dbInited"))
	if err != nil {
		return true, errors.New("failed to read store health status")
	}
...
// mark inited
func (im *Manager) MarkInited() error {

	if err := im.imStore.Set([]byte("dbInited"), []byte{}); err != nil {
		return errors.New("failed to set store health status")
	}
...
// init start status
func initStartIndexKey() []byte {
	m := marshalutil.New(12)
	m.WriteByte(ImStoreKeyPrefixInitStartStatus)
	m.WriteBytes([]byte("initStartIndex"))

	return m.Bytes()
}

// store init start index
func (im *Manager) StoreInitStartIndex(index iotago.MilestoneIndex) error {
	m := marshalutil.New(4)
	m.WriteUint32(index)

	return im.imStore.Set(initStartIndexKey(), m.Bytes())
}

// read init start index
func (im *Manager) ReadInitStartIndex() (iotago.MilestoneIndex, error) {
	v, err := im.imStore.Get(initStartIndexKey())
	if err != nil {
...
// init end status
func initEndIndexKey() []byte {
	m := marshalutil.New(12)
	m.WriteByte(ImStoreKeyPrefixInitEndStatus)
	m.WriteBytes([]byte("initEndIndex"))

	return m.Bytes()
}

// store init end index
func (im *Manager) StoreInitEndIndex(index iotago.MilestoneIndex) error {
	m := marshalutil.New(4)
	m.WriteUint32(index)

	return im.imStore.Set(initEndIndexKey(), m.Bytes())
}

// read init end index
func (im *Manager) ReadInitEndIndex() (iotago.MilestoneIndex, error) {
	v, err := im.imStore.Get(initEndIndexKey())
	if err != nil {
		if errors.Is(err, kvstore.ErrKeyNotFound) {
			return 0, nil
		}
